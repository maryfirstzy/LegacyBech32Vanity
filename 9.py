...
def run_single_thread_vanity_9_char(target_string):
    if len(target_string) != 9:
        raise ValueError("Target string must be exactly 9 characters long.")
        
    # Isolate the 9th character to search for (Difficulty 1)
    target_char = target_string[-1]
    position = 8  # 9th slot in Python indexing (counts from 0)
    ...
    print(f"Targeting Difficulty 1: Any address where character #9 is '{target_char}'")
    print(f"Example shape: 1XXXXXXX{target_char}XXXXX...\n")
    ...
    while True:
        ...
        # Check if the address is long enough and matches the character at the 9th slot
    ...
    log_output = (
        f"--- New 9-Char Variable Address Found ---\n"
...
if __name__ == "__main__":
    # The script ignores '1SmithEL' and actively scans for a 'G' at position 9
    run_single_thread_vanity_9_char("1SmithELG")
